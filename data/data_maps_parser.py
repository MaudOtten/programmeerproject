...
for row in data[1:]:
	...
	if row[0] is not "":
		...
		if float(female_in_research[2]) > 0:
			fillKey = "< 10%"
		if float(female_in_research[2]) < 15:
			fillKey = "< 15%"
		if float(female_in_research[2]) < 20:
			fillKey = "< 20%"
		if float(female_in_research[2]) < 25:
			fillKey = "< 25%"
		if float(female_in_research[2]) < 30:
			fillKey = "< 30%"
		if float(female_in_research[2]) > 30:
			fillKey = "> 30%"
...
outputFile = open('data_maps_2002.json', 'w')
